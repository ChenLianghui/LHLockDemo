...
@interface LHLockListViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation LHLockListViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = NSLocalizedString(@"锁列表", nil);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    LHLockDetailViewController *detailVC = [[LHLockDetailViewController alloc] init];
    LHBaseTableModel *model = self.dataArray[indexPath.row];
...
- (NSMutableArray *)dataArray{
    if (!_dataArray) {
        ...
        for (int i = 0; i < 5; i++) {
            LHBaseTableModel *model = [LHBaseTableModel initBaseModelWithIconName:@"manage_gateway" labelTitle:[NSString stringWithFormat:@"房间%i",i+1] rightTitle:nil isHasSwitch:NO];
