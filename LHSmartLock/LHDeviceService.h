...
- (void)addNewAuthWithLockSN:(NSString *)lock_sn andAuth_user:(NSString *)auth_user andPassword:(NSString *)password andStartTime:(NSDate *)startDate andEndTime:(NSDate *)endDate completed:(void (^)(NSURLSessionTask *task, id responseObject))completed failure:(void (^)(NSURLSessionTask *operation, NSError *error))failure;
